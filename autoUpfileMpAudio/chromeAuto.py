...
for videoName in videoNameList:
    ...
    if len(username_pwd) > 0:
        ...
        upvideoPath = os.path.join(rootdir ,videoName  + '.aac')
        ...
        while len(upvideoBtn) == 0 and not isTimeOut(startTime):
            sleep()
            upvideoBtn = driver.find_elements_by_xpath('//*[@id="creator-main"]/div[1]/div[2]/ul/li[2]/ul/li[2]')
        sleep()
        try:
        ...
        if canUpfile:
            ...
            while len(typeInput) == 0 and not isTimeOut(startTime):
                sleep()
                typeInput = driver.find_elements_by_xpath('//*[@id="audio_info"]/div/div[2]/div/div[2]/form/div[3]/div/div/div[1]/div[1]/input')
            if len(typeInput) > 0:
                typeInput[0].click()
            ...
            while len(keyword) == 0  and not isTimeOut(startTime) :
                keyword = driver.find_elements_by_xpath('//*[@id="audio_info"]/div/div[2]/div/div[2]/form/div[6]/span')
                sleep()
            ...
            keyword = []
            while len(keyword) == 0  and not isTimeOut(startTime) :
                keyword = driver.find_elements_by_xpath('//*[@id="audio_info"]/div/div[2]/div/div[2]/form/div[7]/div/div/div[1]/input')
                sleep()
            ...
            while len(progress_bar) == 0 and not isTimeOut(startTime):
                progress_bar = driver.find_elements_by_xpath('//*[@id="audio_info"]/div/div[2]/div/div[2]/div/div[2]/span[1]')
            ...
            while True and not isTimeOut(startTime):
                try:
                    if driver.find_elements_by_xpath('//*[@id="audio_info"]/div/div[2]/div/div[2]/form/div[8]/div/button[1]')[0].text != '发布':
                        break
                    else:
                        publish = driver.find_elements_by_xpath('//*[@id="audio_info"]/div/div[2]/div/div[2]/form/div[8]/div/button[1]')
                        sleep()
                        if (len(publish) > 0):
                            action = ActionChains(driver).move_to_element(publish[0]).click().perform()
